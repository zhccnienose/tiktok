...
from app.api.schema.interact.fan_list_sha import fan_list_sha
from app.api.utils.res import res


class FanList(Resource):
    def get(self):
        try:
            parser = reqparse.RequestParser()
            parses = fan_list_sha(parser)

            list_uid = [uid.decode("utf-8") for uid in r.smembers(parses['user_id'] + ":set_fans")]
            total = len(list_uid)
            if total == 0:
